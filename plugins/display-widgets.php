...
//        return $instance;
	...
	$verLengua = $instance['lengua-'.ICL_LANGUAGE_CODE];
	if (isset($instance['include']) && (($instance['include'] and $show == false and $verLengua == false ) or ($instance['include'] == 0 and $show and $verLengua)))
		return false;
	else
		return $instance;
...
<div style="height:150px; overflow:auto; border:1px solid #dfdfdf;">
    ...
    foreach($languages as $l ){
        $key = $l['language_code'];
        $instance['lengua-'. $key] = isset($instance['lengua-'. $key]) ? $instance['lengua-'. $key] : false;
    ?>

    <p><input class="checkbox" type="checkbox" <?php checked($instance['lengua-'. $key], true) ?> id="<?php echo $widget->get_field_id('lengua-'. $key); ?>" name="<?php echo $widget->get_field_name('lengua-'. $key); ?>" />
    <label for="<?php echo $widget->get_field_id('lengua-'. $key); ?>"><?php _e($l['native_name']) ?></label></p>
...
function dw_update_widget_options($instance, $new_instance, $old_instance){
    ...
    foreach ($languages as $l)
        $instance['lengua-'.$l['language_code']] = isset($new_instance['lengua-'.$l['language_code'] ]) ? 1 : 0;
