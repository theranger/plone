...
class Pages_Tree_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) { 
		...
		if($k>0) {
			...
			$id = $post->ID;
			if(empty($id)) return;
			if(count(get_pages('sort_column=menu_order&child_of='.$id))==0) return;
		}
		...
		echo '<div class="pages_tree">';
		//If there were any, we are displaying a subpage, get the root and show its children
		
		//wp_list_pages('sort_column=menu_order&title_li=&child_of='.$id);
		flexipages("sort_column=menu_order&child_of=$id&show_subpages=2");
